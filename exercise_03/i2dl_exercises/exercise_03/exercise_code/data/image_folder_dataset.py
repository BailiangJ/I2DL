...
class ImageFolderDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        image_data = self.load_image_as_numpy(self.images[index])
...
def compute_image_mean_and_std(images):
    ...
    assert(mean.shape[0] == images.shape[3])
    assert(std.shape[0] == images.shape[3])
...
class RescaleTransform:
    ...
    def __call__(self, images):
        ...
        old = self._data_max - self._data_min
        k = float(new_range) / float(old)

        images = self.min + k * images
