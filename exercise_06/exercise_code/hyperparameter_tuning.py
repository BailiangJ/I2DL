...
def findBestConfig(train_loader, val_loader, configs, EPOCHS, PATIENCE,
    ...
    for i in range(len(configs)):
        ...
        if not best_val or solver.best_model_stats["val_loss"] < best_val:
            ...
            best_val_acc = solver.get_dataset_accuracy(val_loader)
            best_train_acc = solver.get_dataset_accuracy(train_loader)

    print("\nSearch done. Best Val Loss = {}".format(best_val))
    print("Best train accuracy: %.5f   Best val accuarcy: %.5f" % (best_train_acc, best_val_acc))
