...
class CrossEntropyFromLogits(Loss):
    ...
    def backward(self, y_out, y_truth):
        ...
        # y_truth is one-hot encoding
        # the derivative of softmax function is y_predict * (1 - y_predict), y_predict is the predicted probability
        # CE = 1/N * sum^(i and k) of (- y_truth_ik * log(y_predict_ik)
        # y_truth_ik = 1 iff the true label of the ith sample is k and 0 otherwise
        # so the derivative is
        #   y_predict_correct * (1 - y_predict_correct) * (-1 * 1/y_predict_correct)
        # = -(1 - y_predict_correct)
