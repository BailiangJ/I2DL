...
class MyOwnNetwork(ClassificationNet):
    ...
    def __init__(self, activation=Sigmoid(), num_layer=2,
                 input_size=3 * 32 * 32, hidden_size=100,
                 std=1e-3, num_classes=10, reg=0, dropout=0.5, **kwargs):
        """
        ...
        for i in range(num_layer - 2):
            self.params['W' + str(i + 2)] = np.random.randn(hidden_size, hidden_size) * np.sqrt(2 / hidden_size)
            self.params['b' + str(i + 2)] = np.zeros(hidden_size)

        self.params['W' + str(num_layer)] = np.random.randn(hidden_size, num_classes) * np.sqrt(2 / hidden_size)
        self.params['b' + str(num_layer)] = np.zeros(num_classes)
        ...
        for i in range(num_layer - 1):
            self.dropout['D' + str(i + 1)] = np.random.binomial(n=1, p=self.keep_prob,
                                                                size=hidden_size) / self.keep_prob
